...
export const getLengthOfCategoryPosts = async (req, res) => {
  try {
    ...
    posts.forEach((post) => {
      if (post.categories === "fashion") {
        categories.fashion.push(post);
      }
      if (post.categories === "travel") {
        categories.travel.push(post);
      }
      if (post.categories === "health") {
        categories.health.push(post);
      }
      if (post.categories === "education") {
        categories.education.push(post);
      }
    });

    res.status(200).json({
      fashion: categories.fashion.length,
      travel: categories.travel.length,
...
export const topCategoryPosts = async (req, res) => {
  try {
    ...
    res.status(200).json({
      travel: [...posts.travel][0] || {},
      education: [...posts.education][0] || {},
      news: [...posts.news][0] || {},
      adventure: [...posts.adventure][0] || {},
      entertainment: [...posts.entertainment][0] || {},
